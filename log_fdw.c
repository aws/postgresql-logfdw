               ...
               char **filename, List **other_options)
{
    ...
    *filename = full_filename;
    ...
    else
    {
         ...
         * cause problems if the tab character is used in the log file.  To get
         * around this, set the delimiter to be the EOT character.  This is a
         * much rarer character to encounter in a log file, so it should
...
fileIterateForeignScan(ForeignScanState *node)
{
     ...
     * But if still use log_fdw 1.1 to read a log file part of which have this
     ...
     * customer about why it happens by catching these 2 errors. 
     * Notice that some other problems like wrong format for some certain
     * columns will be catched as well, for which we did not replace
     * original error message and error hints, as their errors are irrelevant 
     * with "backend_type" issue, thus we just reuse existing hints.
     * Moreover this code change is generically applicable to any log formatting
     * change in the future, which we should keep an eye on it for future
     * update. 
     */
    PG_TRY();
    {
